...
export const SidebarContainer = styled.div`
    ...
    background-image: linear-gradient(
        ...
        url(${p => p.backgroundImage}); //props kan hedde hver som helst
...
export const MenuItem = styled.div`
        //Only center icons if the sidebar is collapsed
        ${p => !p.isSidebarOpen && `
                text-align: center;
        ...
        padding: 6px 20px;
        font-weight: 600;
        // if selected display white color or else remain the color before
        color: ${p => p.selected ? 'rgba(255, 255, 255)' : 'rgba(19, 15, 64)'};
        font-family: ${p => p.font};
        // text stays in the same line as icon
        white-space: nowrap;
        ...
        &:after {
                content: '';
                // if selected display white color or else remain the color before
                border: 1px solid ${p => p.selected ? 'rgba(255, 255, 255)' : 'rgba(255,112,85)'};
                
                // if isSidebarOpen & selected & open display none or else display block (the underline/divider under Destinations)
...
export const Text = styled.p`
        // if the sidebar is open show inline text, else only icons
        display: ${p => p.isSidebarOpen ? 'inline' : 'none'};
`

export const Icon = styled.img`
        //if the sidebar is not collapsed add padding-right by 20px
